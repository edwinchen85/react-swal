...
class SweetAlert extends React.Component {
  ...
  render() {
    let isOpen = Immutable.fromJS(this.props.isOpen);

    if (isOpen) {
      let options = Immutable.fromJS(pick(this.props, Object.keys(SweetAlert.defaultProps)));
      swal(options, result => {
